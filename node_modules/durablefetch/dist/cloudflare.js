...
export default {
    async fetch(req, env) {
        ...
        if (url.pathname === '/') {
            return Response.redirect('https://github.com/remorses/durablefetch', 307);
        }
        // special case for the README demo
        if (url.searchParams.get('randomId') === 'xxxx') {
            url.searchParams.set('randomId', Math.random().toString(36).substring(2, 18));
            return new Response('', {
                status: 307,
                headers: {
...
export class DurableFetch extends DurableObject {
    state;
    seq = 0; // next chunk index
    fetching = false; // upstream started?
    live = new Set();
    // Time To Live (TTL) in milliseconds
    timeToLiveMs = 60 * 60 * 1000 * 6;
    constructor(state) {
        super(state, {});
        this.state = state;
        ...
        this.state.blockConcurrencyWhile(async () => {
            ...
            this.fetching =
                (await this.state.storage.get('open')) ?? false;
        });
    }
    /* ------------------------------------------------------ */
    async fetch(req) {
        ...
        const isCompleted = await this.state.storage.get('completed');
        let headers;
        ...
        if (!this.fetching && !isCompleted) {
            const maybeRes = await this.state.blockConcurrencyWhile(async () => {
    ...
    async checkInProgress() {
        ...
        const completed = (await this.state.storage.get('completed')) ?? false;
    ...
    async pipeUpstream(res) {
        try {
            ...
            while (true) {
                ...
                await this.state.storage.put(`c:${this.seq.toString().padStart(9, '0')}`, value);
                this.seq++;
    ...
    async deleteStorage() {
        ...
        this.fetching = false;
        this.live.clear();
        return new Response(JSON.stringify({ success: true, message: 'Storage cleared' }), {
            headers: {
                'content-type': 'application/json',
                ...getCorsHeaders(),
            },
...
async function createDurableObjectRequest(url, req) {
