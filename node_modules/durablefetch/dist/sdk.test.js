...
describe('DurableFetchClient', () => {
    ...
    it('should fetch first events from stream', async () => {
        ...
        const events = await collectGeneratorUntil(iterator1, iterationInterval);
    ...
    it('should resume stream from third event', async () => {
        ...
        const events = await collectGeneratorUntil(iterator2, iterationInterval);
        console.log('aborting');
        controller2.abort();
        expect(events.map((e) => e.data)).toMatchInlineSnapshot(`
              [
    ...
    it('should continue resuming stream and get the last event', async () => {
        ...
        const events = await collectGeneratorUntil(iterator3, iterationInterval);
        console.log('aborting');
        controller3.abort();
        expect(events.map((e) => e.data)).toMatchInlineSnapshot(`
              [
 ...
 */
async function collectGeneratorUntil(generator, ms) {
    const items = [];
 ...
 */
async function* streamSseEvents(stream) {
