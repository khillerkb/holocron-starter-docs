...
import { getCurrentGitBranch, openUrlInBrowser, readTopLevelDocsJson, safeParseJson, getGitRepoRoot, getGitHubInfo, checkGitStatus, DOCS_JSON_BASENAME, isDocsJson, } from './utils.js';
...
async function findProjectFiles(workingDir = process.cwd()) {
...
async function uploadMediaFiles({ mediaFilePaths, siteId, }) {
...
async function determineTemplateDownload({ markdownFileCount, fromTemplateFlag, isInteractive, }) {
    if (fromTemplateFlag) {
        return true;
    }
...
cli
    ...
    try {
        ...
        let existingDocsJson = undefined;
        try {
        ...
        const mediaFilesWithMetadata = await Promise.all(mediaFilePaths.map(async (x) => {
        ...
        table.push(['Site Name', siteName], ['Website URL', websiteUrl], [
            'Files Uploaded',
            `${filePaths.length} text files${mediaFilePaths.length > 0 ? `, ${mediaFilePaths.length} media files` : ''}`,
        ], ['GitHub', `${data.githubOwner}/${data.githubRepo}/${data.githubFolder}`], ['Branch', gitBranch || 'main']);
...
cli
    ...
    try {
        ...
        await new Promise((resolve) => {
            const cb = (filePath) => {
                const isTextFile = filePath.endsWith('.mdx') ||
        ...
        if (!docsJson) {
            console.error(pc.red(`${DOCS_JSON_BASENAME} file not found at the project root. Use holocron init to create a new project`));
            process.exit(1);
        }
        ...
        const previewUrl = new URL(previewDomain.includes('.localhost:') || previewDomain.endsWith('.localhost')
        ...
        const filesInDraft = Object.fromEntries(await Promise.all(filePaths.map(async (filePath) => {
        ...
        const handleFileUpdate = async (filePath, revalidate) => {
            const fullPath = path.resolve(dir, filePath);
            // Check if it's a media file
            const isMediaFile = MEDIA_EXTENSIONS.some((ext) => filePath.toLowerCase().endsWith(`.${ext}`));
            // For media files, use empty content (they're handled separately)
...
cli.command('logout', 'Delete user token and config').action(async () => {
    try {
        ...
        console.error(pc.red('Error during logout: ' + (error.message || error)));
        process.exit(1);
    }
});
